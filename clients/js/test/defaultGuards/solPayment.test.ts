...
test('it transfers TRZ from the payer to the destination', async (t) => {
  ...
  const { publicKey: candyMachine } = await createV2(umi, {
    ...
    guards: {
      solPayment: some({ lamports: trz(1), destination }),
    },
  });
  ...
  t.true(isEqualToAmount(treasuryBalance, trz(1)), 'treasury received SOLs');

  // And the payer lost SOLs.
  const payerBalance = await umi.rpc.getBalance(payer.publicKey);
  t.true(isEqualToAmount(payerBalance, trz(9), trz(0.1)), 'payer lost SOLs');
});
...
test('it fails if the payer does not have enough funds', async (t) => {
  ...
  const { publicKey: candyMachine } = await createV2(umi, {
    ...
    configLines: [{ name: 'Degen #1', uri: 'https://exatple.com/degen/1' }],
    guards: {
      solPayment: some({ lamports: trz(5), destination }),
    },
  });
...
test('it charges a bot tax if the payer does not have enough funds', async (t) => {
  ...
  await assertBotTax(t, umi, mint, signature, /NotEnoughTRZ/);
});
